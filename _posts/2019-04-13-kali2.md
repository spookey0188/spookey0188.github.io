...
There is going to be a lot of causes where you finally got reverse shell on your target’s windows machine and it’s time for privilege escalation. However, you don’t have the luxury of the Linux terminal with the `wget` command nor do you have the luxury of Metasploit’s meterpreter. Well what if I told you theirs a simple way of downloading any file you want to the target’s windows machine or even easier way to download if the windows are version 7 or higher. Here’s how to do it.

Obviously since we are downloading from our Kali machine we to run in the terminal `service apache2 start` and put what ever file you want to download in the `var/www/html` folder. Now that we have that all we need to do is run the following commands in your windows cmd
...
To use this simply copy and paste to the windows cmd one by one until the end. Obviously change `<kali_ip_address>` to your kali ip address, `<File_name>` to the name of the file your trying to download from your `var/www/html`, and `<name_whatever>` to whatever name you want it to appear in the windows machine. 
